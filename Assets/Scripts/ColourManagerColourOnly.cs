...
public class GameMoveCO
{
    public int[] positions;
    public int WiningPosition;
}

public static class SpawnSettingsCO
{
...
public class ColourManagerColourOnly : MonoBehaviour
{
    ...
    List<GameMoveCO> _moves;
    GameMoveCO _lastMove;
    int _activemove;
    ...
    void Start()
    {
        highScore = PlayerPrefs.GetInt("hardHS", 0);

        SpawnSettingsCO.spawnArray = spawnArray;
        SpawnSettingsCO.listToSpawn = colourArray;
        SpawnSettingsCO.spawnedList = new GameObject[4];
    ...
    void _randomizeMoves(int level)
    {
        ...
        _moves = new List<GameMoveCO>(level);
    ...
    public IEnumerator SpawnMove(GameMoveCO Move, float timer) //show round
    {
        shuffleArrayObjectPositions(SpawnSettingsCO.spawnArray);
        goingDown = false;
        for (int i = 0; i < spawnArray.Length; i++)
        {
    ...
    {
        if (SpawnSettingsCO.spawnedList[position] != null)
        {
            Destroy(SpawnSettingsCO.spawnedList[position].gameObject);
        }

        GameObject newMole = Instantiate(SpawnSettingsCO.listToSpawn[MoleID].gameObject, SpawnSettingsCO.spawnArray[position].transform.position, transform.rotation) as GameObject;
        audioManager.playMoleAppear();
        if (newMole.GetComponent<BoxCollider2D>())
        {
            newMole.GetComponent<BoxCollider2D>().enabled = false;
        }
        SpawnSettingsCO.spawnArray[position].GetComponent<Spawner_Controller>().currentMole = newMole;
        ...
        SpawnSettingsCO.spawnedList[position] = newMole;
    }
    ...
    public IEnumerator DestroyElement(int position)
    {
        ...
        GameObject mole = SpawnSettingsCO.spawnedList[position];
        ...
        SpawnSettingsCO.spawnedList[position] = null;
    }
    ...
    public void losePanel(GameObject wrongMole, GameObject correctMole)
    {
        ...
        correctMoleText.text = "I probably moved to another spot! I was the correct choice";
        wrongMoleText.text = "Oops, wrong choice!";
    ...
    {
        for (int i = 0; i < SpawnSettingsCO.spawnedList.Length; i++)
        {
            GameObject mole = SpawnSettingsCO.spawnedList[i].gameObject;
            if (mole.transform.childCount > 0)
            {
                if (mole.transform.GetChild(0).GetComponent<Mole>())
                {
        ...
        //for (int i = 0; i < SpawnSettingsCO.spawnedList.Length; i++)
        //{
        //    if (SpawnSettingsCO.spawnedList[i] != null)
        //    {
        //        GameObject mole = SpawnSettingsCO.spawnedList[i].gameObject;
        //        if (mole.transform.childCount > 0)
        //        {
    ...
    {
        PlayerPrefs.SetInt("hardHS", highScore);
        PlayerPrefs.Save();
    }
    ...
    public void disableAllActiveColliders()
    {
        for (int i = 0; i < SpawnSettingsCO.spawnedList.Length; i++)
        {
            if (SpawnSettingsCO.spawnedList[i].gameObject.GetComponent<BoxCollider2D>())
            {
                SpawnSettingsCO.spawnedList[i].gameObject.GetComponent<BoxCollider2D>().enabled = false;
            }
        }
    }
}
